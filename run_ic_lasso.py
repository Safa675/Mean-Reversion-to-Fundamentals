...
def main():
    ...
    selected = select_features_by_ic(
        ...
        candidate_pool,
        model_type="lasso",
        sector_label="Pooled IC-Lasso",
        min_features=5,
    )
    print(f"[RESULT] IC-Lasso selected features: {selected}")
    ...
    payload = run_pooled_ols(
        df_common,
        selected,
        model_key="Pooled_OLS_IC_Lasso",
        model_name=f"Pooled OLS (IC-Lasso, {len(selected)} vars)",
    )
    if payload is None:
        print("[ERROR] OLS run failed.")
    else:
        print("[OK] IC-Lasso OLS run complete.")
