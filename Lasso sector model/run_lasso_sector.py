...
from pooled_ols_residuals_bist import (  # noqa: E402
    TARGET_COL,
    prepare_panel_data,
...
OUTPUT_DIR = ROOT / "Lasso sector model"
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
OUTPUT_XLSX = OUTPUT_DIR / "bist_lasso_sector_models.xlsx"
...
def run_sector(df_p, code):
    ...
    payload = run_pooled_ols(
        df_common,
        selected,
        model_key=f"{code}_Lasso",
        model_name=f"{code} Lasso (IC)",
    )
    return payload
...
def main():
    ...
    if selected_all:
        ...
        payload = run_pooled_ols(
            df_common_all,
            selected_all,
            model_key="Pooled_Lasso",
            model_name=f"Pooled Lasso (IC, {len(selected_all)} vars)",
        )
        if payload:
    ...
    if not payloads:
        print("[ERROR] No Lasso IC models ran.")
        return
    ...
    print(f"[OK] Lasso sector models saved to {OUTPUT_XLSX}")
