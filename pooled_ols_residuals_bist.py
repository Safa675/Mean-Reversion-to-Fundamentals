...
def cross_sectional_zscore(series: pd.Series, group_index: pd.Series) -> pd.Series:
    """
    Compute per-group z-score to avoid time-drift (e.g., inflation-driven growth).
    If std within group is zero/NaN, return 0 for that group.
    """
    grouped = series.groupby(group_index)
    mean = grouped.transform("mean")
    std = grouped.transform("std")
    z = (series - mean) / std
    z = z.mask(std.isna() | (std == 0), 0.0)
    return z
...
def build_bubble_outputs(model_name, y_log_mcap, predicted_log_mcap, base_df):
    ...
    overvaluation_pct = (actual_mcap / predicted_mcap - 1) * 100
...
def add_forward_returns(df, horizons=(1, 2, 4), value_col="MarketCap"):
    """Append forward returns in percent for specified horizons."""
    if value_col not in df.columns:
        raise ValueError(f"{value_col} not in dataframe for forward returns.")
    df = df.sort_values(["Ticker", "QuarterDate"])
    for h in horizons:
        fwd_col = f"{value_col}_forward_{h}Q"
        df[fwd_col] = df.groupby("Ticker")[value_col].shift(-h)
        base = df[value_col]
        fwd = df[fwd_col]
        valid = (base > 0) & (fwd > 0)
...
def prepare_panel_data():
    ...
    # Log-transform target WITHOUT global clipping; keep raw for returns
    df_edgar[TARGET_COL] = np.log(df_edgar["MarketCap"].clip(lower=1e3))

    # Replace inf/nan before scaling
    df_edgar = df_edgar.replace([np.inf, -np.inf], np.nan)

    # Cross-sectional winsorize + z-score per QuarterDate to reduce regime drift
    print("\nCross-sectional winsorizing and z-scoring by QuarterDate...")
    for col in feature_cols:
        if col not in df_edgar.columns:
            continue
        # Winsorize within each quarter
        grouped = df_edgar.groupby("QuarterDate")[col]
        def _winsor(s):
            if len(s.dropna()) < 5:
                return s
            lower, upper = s.quantile([0.01, 0.99])
            return s.clip(lower=lower, upper=upper)
        df_edgar[col] = grouped.transform(_winsor)
        # Per-quarter z-score
        df_edgar[f"{col}_z"] = cross_sectional_zscore(df_edgar[col], df_edgar["QuarterDate"])
...
def _filter_feature_coverage(df, candidate_features, min_features=3):
    total = len(df)
...
def run_lasso_feature_selection(df, candidate_features, sector_label="ALL", min_features=3):
...
def evaluate_ic_for_params(df, features, model_cls, model_kwargs, horizon_col="Return_4Q", n_splits=4):
    """Cross-validated IC for a given alpha/params using TimeSeriesSplit."""
    required_cols = [TARGET_COL, horizon_col]
    df_sub = df.dropna(subset=required_cols).copy()
    if df_sub.empty:
        return np.nan
    df_sub = df_sub.sort_values("QuarterDate")

    X = df_sub[features].copy()
    X = X.apply(lambda col: col.fillna(col.median()))
    y = df_sub[TARGET_COL].values
    r = df_sub[horizon_col].values

    scaler = StandardScaler()
    X_scaled = scaler.fit_transform(X)

    max_splits = max(2, min(len(df_sub) - 1, max(2, len(df_sub) // 100)))
    n_splits = min(n_splits, max_splits)
    if n_splits < 2 or len(df_sub) <= n_splits:
        return np.nan
    splitter = TimeSeriesSplit(n_splits=n_splits)
    ics = []
    for train_idx, val_idx in splitter.split(X_scaled):
        ...
        overval = (y_val - y_pred) * 100  # log residual in pct terms
        mask = ~np.isnan(overval) & ~np.isnan(r_val)
        if mask.sum() == 0:
            continue
        ic, _ = spearmanr(overval[mask], r_val[mask])
        ics.append(ic)
...
def select_features_by_ic(df, candidate_features, model_type="lasso", l1_grid=None, alpha_grid=None,
                          horizon_col="Return_4Q", min_features=3, max_features=20, sector_label="ALL"):
    """Pick features that maximize return IC using ElasticNet/Lasso grid search."""
    if df.empty:
        print(f"[WARN] IC selection skipped for {sector_label}: empty dataframe")
        return []

    usable, coverage = _filter_feature_coverage(df, candidate_features, min_features=min_features)
    if not usable:
        print(f"[WARN] IC selection usable features empty for {sector_label}")
        return []

    # Precompute forward returns if missing
    if horizon_col not in df.columns:
        df = add_forward_returns(df, horizons=(1, 2, 4))

    # Defaults
    alpha_grid = alpha_grid or np.logspace(-3, 1, 10)
    l1_grid = l1_grid or ([1.0] if model_type == "lasso" else [0.1, 0.3, 0.5, 0.7, 0.9])

    best = {"ic": -np.inf, "alpha": None, "l1": None, "model": None}
    for l1 in l1_grid:
        for alpha in alpha_grid:
            kwargs = {"alpha": alpha, "max_iter": 5000, "l1_ratio": l1, "fit_intercept": True}
            model_cls = ElasticNet
            ic = evaluate_ic_for_params(df, usable, model_cls, kwargs, horizon_col=horizon_col)
            print(f"[IC] {sector_label} l1={l1:.2f} alpha={alpha:.4f} -> mean IC={ic:.4f}")
            if np.isnan(ic):
                continue
            if ic < best["ic"]:
                continue
            best.update({"ic": ic, "alpha": alpha, "l1": l1})

    if best["alpha"] is None:
        print(f"[WARN] No valid IC scores for {sector_label}; returning coverage-based head")
        return usable[:min_features]

    print(f"[OK] Best IC params for {sector_label}: l1={best['l1']:.2f}, alpha={best['alpha']:.4f}, IC={best['ic']:.4f}")

    # Fit on full data to extract strongest coefficients
    df_sub = df.dropna(subset=[TARGET_COL]).copy()
    df_sub = add_forward_returns(df_sub, horizons=(1, 2, 4))
    df_sub = df_sub.dropna(subset=[horizon_col])
    if df_sub.empty:
        return usable[:min_features]

    X_full = df_sub[usable].apply(lambda col: col.fillna(col.median()))
    scaler = StandardScaler()
    X_scaled = scaler.fit_transform(X_full)
    model = ElasticNet(alpha=best["alpha"], l1_ratio=best["l1"], max_iter=5000, fit_intercept=True)
    model.fit(X_scaled, df_sub[TARGET_COL].values)
    coef_pairs = list(zip(usable, model.coef_))
    coef_pairs.sort(key=lambda x: abs(x[1]), reverse=True)
    selected = [feat for feat, coef in coef_pairs if abs(coef) > 1e-6]
    if not selected:
        selected = [feat for feat, _ in coef_pairs[:min_features]]
    selected = selected[:max_features]
    if len(selected) < min_features:
        selected = selected + [feat for feat, _ in coef_pairs if feat not in selected][: min_features - len(selected)]
        selected = selected[:max_features]
    print(f"[SELECTED] {sector_label}: {len(selected)} features -> {selected[:15]}{' ...' if len(selected)>15 else ''}")
    return selected
