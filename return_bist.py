...
RESULTS_PATH = ROOT / "bist_bubbleness_analysis_pooled_ols.xlsx"
MARKET_CAP_PATH = ROOT / "all_tickers_quarterly_2016_2026.csv"
SHEET_NAME = "Pooled_OLS_All"

OUTPUT_RESULTS_TEMPLATE = "bist_mean_reversion_analysis_{sector}.xlsx"
OUTPUT_SECTOR_EXTREMES = ROOT / "bist_sector_extremes_last_year.xlsx"
OUTPUT_PLOT = ROOT / "bist_mean_reversion_by_overvaluation.png"
RECENT_QUARTERS = 4
...
def load_results_and_market_data(sheet_name=SHEET_NAME, sector_code=None):
    """Load the bubble analysis results and market cap data."""
...
def compute_forward_returns(df_results, df_mc, horizons=(1, 2, 4), value_col="MarketCap"):
    ...
    horizons in quarters (1Q, 2Q, 4Q = 1 year)
    """
    df = df_results.merge(
...
def print_recent_sector_extremes(df, n_quarters=RECENT_QUARTERS, top_n=SECTOR_TOP_N):
    """Print undervalued names by sector for the most recent quarters (suppress overvalued)."""
...
def create_overvaluation_groups(df, n_groups=5):
    """
    Create quintiles based on overvaluation percentage
    1 = Most Undervalued, 5 = Most Overvalued
    """
...
def analyze_mean_reversion(df, horizons=(1, 2, 4)):
    ...
    for h in horizons:
        ...
        quintile_analysis = df.groupby("Overval_Quintile").agg({
        ...
        decile_analysis = df.groupby("Overval_Decile").agg({
...
def plot_mean_reversion(df, horizons=(1, 2, 4)):
    ...
    fig.suptitle(
        "Mean Reversion Analysis: Do Overvalued Stocks Underperform?",
        fontsize=16,
        fontweight="bold",
    )
...
def export_results(df, quintile_analysis, decile_analysis, correlation_summary, decile_by_year_4q,
    ...
    output_path = output_path or (ROOT / OUTPUT_RESULTS_TEMPLATE.format(sector="all"))
    with pd.ExcelWriter(output_path, engine="openpyxl") as writer:
        ...
        if not decile_by_year_4q.empty:
            decile_by_year_4q.to_excel(writer, sheet_name=FOUR_QUARTER_SHEET, index=False)

        top_overvalued = df.nlargest(20, "Overvaluation_pct")[
            ["Ticker", "QuarterDate", "Overvaluation_pct", "Return_4Q"]
        ]
        top_overvalued.to_excel(writer, sheet_name="Top_Overvalued", index=False)

        bottom_undervalued = df.nsmallest(20, "Overvaluation_pct")[
            ["Ticker", "QuarterDate", "Overvaluation_pct", "Return_4Q"]
        ]
        bottom_undervalued.to_excel(writer, sheet_name="Top_Undervalued", index=False)

        if sector_under is not None and not sector_under.empty:
            sector_under.to_excel(writer, sheet_name="Sector_Undervalued_LastYear", index=False)
        if sector_over is not None and not sector_over.empty:
            sector_over.to_excel(writer, sheet_name="Sector_Overvalued_LastYear", index=False)
...
def main():
    print("=" * 70)
    print("MEAN REVERSION VALIDATION TEST (BIST)")
    print("Testing if fundamental overvaluation predicts future underperformance")
    print("=" * 70)

    # Load market data once
    print("\n[1/7] Loading market data...")
    _, df_mc, value_col = load_results_and_market_data()

    all_sector_under = []
    all_sector_over = []

    for idx, code in enumerate(SECTOR_CODES, 1):
        sheet = f"{code}_OLS_All"
        print(f"\n{'='*70}\nProcessing sector {code} (sheet: {sheet}) [{idx}/{len(SECTOR_CODES)}]")
        try:
            df_results, _, _ = load_results_and_market_data(sheet_name=sheet, sector_code=code)
        except Exception as exc:
            print(f"[WARN] Skipping sector {code}: {exc}")
            continue

        print(f"Loaded {len(df_results)} observations for sector {code}")
        ...
        df["Year"] = pd.to_datetime(df["QuarterDate"], errors="coerce").dt.year
        ...
        quintile_analysis, decile_analysis, correlation_summary = analyze_mean_reversion(
            df, horizons=(1, 2, 4)
        )

        print("[Step] Decile-by-year 4Q returns for export...")
        ...
        if not sector_under.empty:
            sector_under["Sector"] = sector_under["Sector"].replace("", code)
            all_sector_under.append(sector_under)
        if not sector_over.empty:
            sector_over["Sector"] = sector_over["Sector"].replace("", code)
            all_sector_over.append(sector_over)

        print("[Step] Creating visualizations...")
        plot_mean_reversion(df, horizons=(1, 2, 4))

        output_path = ROOT / OUTPUT_RESULTS_TEMPLATE.format(sector=code.lower())
        export_results(
            df,
            quintile_analysis,
            decile_analysis,
    ...
    # Export consolidated last-year extremes across sectors
    if all_sector_under or all_sector_over:
        under_df = pd.concat(all_sector_under, ignore_index=True) if all_sector_under else pd.DataFrame()
        over_df = pd.concat(all_sector_over, ignore_index=True) if all_sector_over else pd.DataFrame()
        with pd.ExcelWriter(OUTPUT_SECTOR_EXTREMES, engine="openpyxl") as writer:
            if not under_df.empty:
                under_df.to_excel(writer, sheet_name="Undervalued_LastYear", index=False)
            if not over_df.empty:
                over_df.to_excel(writer, sheet_name="Overvalued_LastYear", index=False)
        print(f"\n[OK] Sector extremes exported to {OUTPUT_SECTOR_EXTREMES}")
    ...
    print(
        """
What we're testing:
- Each sector regression is analyzed separately (CAP, COM, CON, FIN, IND, INT, RE)
- Overvalued stocks (Q5/D10) should underperform undervalued stocks (Q1/D1) if mean reversion holds

Key metrics to check:
1. Quintile plot slope (Q1 > Q5?)
2. Long-short returns (D1 - D10) positive?
3. Correlation p-value < 0.05?
        """.strip()
    )
